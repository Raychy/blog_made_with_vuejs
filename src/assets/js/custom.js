...
var LeMars = function(){
	...
	var fileInput = function(){
		...
		jQuery(document).on('change', '.btn-file :file', function() {
			...
			var	label = input.val().replace(/\\/g, 'https://lemars.dexignzone.com/').replace(/.*\//, '');
		...
		jQuery(window).on('scroll', function () {
			if(jQuery('.sticky-header').length > 0){
				...
				if ($(window).scrollTop() > menu.offset().top) {
					...
					$('.header-style-5 .container > .logo-header .logo').attr('src','http://lemars.dexignzone.com/xhtml/error-404.html')
				}
			}
		});
